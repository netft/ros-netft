...
struct AdapterParameters {
  ...
  double configuration_connect_timeout{0.5};
  double configuration_timeout{1.0};
...
inline AdapterConfig map_adapter_parameters(const AdapterParameters & parameters)
{
  ...
  mapped.client.rdt_port = parameters.sensor_port;
  mapped.client.http_port = parameters.http_port;
  mapped.client.receive_timeout = std::chrono::duration<double>{parameters.receive_timeout};
  mapped.client.configuration_connect_timeout =
    std::chrono::duration<double>{parameters.configuration_connect_timeout};
  mapped.client.configuration_timeout =
    std::chrono::duration<double>{parameters.configuration_timeout};
  ...
  mapped.client.sample_rate_limit_hz = parameters.publish_rate;
  mapped.client.deliver_samples_with_error_status = parameters.publish_on_error;
  if (!parameters.use_sensor_calibration) {
    mapped.client.calibration_override = netft::Calibration{
      parameters.counts_per_force,
      parameters.counts_per_torque,

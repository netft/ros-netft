...
// Derived from the vendored netft-cpp snapshot; see ../core/UPSTREAM and ../core/LICENSE.
...
bool is_general_decimal(std::string_view value) noexcept {
  std::size_t position{};
  ...
  while (position < value.size() && is_ascii_digit(value[position])) {
...
double parse_positive_count(std::string_view value, std::string_view tag) {
  ...
  if (!is_general_decimal(value) || input.fail() ||
      input.rdbuf()->sgetc() != std::char_traits<char>::eof() || !std::isfinite(result) ||
      result <= 0.0) {
